...
const ResumeWorkoutModal = ({ activeSession, onResume, onDiscard }) => {
  ...
  return (
    <Modal ariaLabel="Resume workout" z={Z_RESUME_PROMPT} cardClassName="max-w-sm p-6">
      ...
      <p className="text-body mb-8 text-ink/45">{t('modals.setsCompleted', { completed, total })}</p>
      <button onClick={onResume} className="w-full h-12 flex items-center justify-center rounded-lg border border-accent text-accent font-medium text-[14.5px] active:scale-95 mb-6">{t('modals.resume')}</button>
      <button onClick={onDiscard} className="w-full min-h-[44px] flex items-center justify-center text-card active:scale-90 text-ink/45">{t('modals.discard')}</button>
    </Modal>
  );
};
