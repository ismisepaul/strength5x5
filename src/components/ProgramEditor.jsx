...
const ProgramEditor = ({ program, onChange, isDark, isWorkoutActive }) => {
  ...
  const stepperClass = `w-10 h-10 rounded-lg border flex items-center justify-center shrink-0 transition-colors hover:border-accent ${isDark ? 'border-ink/18 text-ink/60' : 'border-ink-lt/18 text-ink-lt/60'}`;

  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-[22px] font-medium">{t('program.title')}</h2>
        <button
          onClick={() => onChange(() => JSON.parse(JSON.stringify(DEFAULT_PROGRAM)))}
          className={`flex items-center gap-1.5 text-[10px] uppercase px-3 py-2 rounded-lg border active:scale-95 transition-transform ${isDark ? 'border-ink/18 text-ink/60' : 'border-ink-lt/18 text-ink-lt/60'}`}
        ><ArrowCounterClockwise size={12} /> {t('program.resetDefaults')}</button>
      </div>
      <p className={`text-xs leading-relaxed -mt-1 ${mutedClass}`}>{t('program.subtitle')}</p>
      {isWorkoutActive && (
        <p className={`text-xs leading-relaxed ${mutedClass}`}>{t('program.activeWorkoutNote')}</p>
      )}
        ...
        return (
          <div key={id} className={`p-4 rounded-[10px] border ${isDark ? 'bg-surface border-ink/8' : 'bg-surface-lt border-ink-lt/8'}`}>
            <div className="flex justify-between items-center mb-4">
              <p className="text-sm font-semibold">{t('exercises.' + id)}</p>
              <p className={`text-[11.5px] ${isDark ? 'text-ink/50' : 'text-ink-lt/50'}`}>
                {t('program.summary', { sets: t('program.setsCount', { count: sets }), reps: t('program.repsCount', { count: reps }) })}
              </p>
            </div>

            <div className="flex items-center gap-3 mb-4">
              <span className={`w-9 text-[10px] uppercase ${mutedClass}`}>{t('program.setsLabel')}</span>
              ...
              <span className="w-6 text-center text-[17px] tabular-nums">{sets}</span>
              <button
                onClick={() => update(id, 'sets', 1, MIN_SETS, MAX_SETS)}
            ...
            <div className="flex items-center gap-3">
              <span className={`w-9 text-[10px] uppercase ${mutedClass}`}>{t('program.repsLabel')}</span>
              <div className="flex-1 flex gap-1">
              ...
              <button
                ...
                className={stepperClass}
              ><Minus size={16} /></button>
              <span className="w-6 text-center text-[17px] tabular-nums">{reps}</span>
              <button
                onClick={() => update(id, 'reps', 1, MIN_REPS, MAX_REPS)}
